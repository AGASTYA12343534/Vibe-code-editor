...
const TemplateSelectionModal = ({
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[850px] max-h-[90vh] overflow-y-auto bg-[#0A0A0A] border-white/10 text-white rounded-xl shadow-[0_0_60px_rgba(0,0,0,0.5)]">
        {step === "select" ? (
          <>
            <DialogHeader className="border-b border-white/10 pb-4 mb-2">
              <DialogTitle className="text-2xl font-bold text-white flex items-center gap-3">
                <div className="p-2 bg-[#E93F3F]/10 rounded-lg border border-[#E93F3F]/20">
                  <Code size={20} className="text-[#e93f3f]" />
                </div>
                Initialize Environment
              </DialogTitle>
              <DialogDescription className="text-zinc-400 text-sm mt-1">
                Select a foundational stack for your new training session
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex flex-col gap-6 py-4">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1 group">
                  <Search
                    className="absolute left-3 top-1/2 transform -translate-y-1/2 text-zinc-500 group-focus-within:text-[#E93F3F] transition-colors"
                    size={18}
                  />
                  <Input
                    ...
                    className="pl-10 bg-[#111111] border-white/10 focus-visible:ring-[#E93F3F] focus-visible:border-[#E93F3F] text-white"
                  />
                </div>
                ...
                >
                  <TabsList className="grid grid-cols-4 w-full sm:w-[400px] bg-[#111111] border border-white/5 p-1 rounded-lg">
                    <TabsTrigger value="all" className="data-[state=active]:bg-[#222] data-[state=active]:text-white text-zinc-400">All</TabsTrigger>
                    <TabsTrigger value="frontend" className="data-[state=active]:bg-[#222] data-[state=active]:text-white text-zinc-400">Frontend</TabsTrigger>
                    <TabsTrigger value="backend" className="data-[state=active]:bg-[#222] data-[state=active]:text-white text-zinc-400">Backend</TabsTrigger>
                    <TabsTrigger value="fullstack" className="data-[state=active]:bg-[#222] data-[state=active]:text-white text-zinc-400">Fullstack</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredTemplates.length > 0 ? (
                    filteredTemplates.map((template) => (
                      <div
                        key={template.id}
                        className={`relative flex p-6 border rounded-xl cursor-pointer transition-all duration-300 hover:-translate-y-1
                          ${
                            selectedTemplate === template.id
                              ? "border-[#E93F3F] bg-[#E93F3F]/5 shadow-[0_0_0_1px_#E93F3F,0_8px_20px_rgba(233,63,63,0.15)]"
                              : "border-white/10 bg-[#111111] hover:bg-[#161616] hover:border-white/20 shadow-lg"
                          }
                          `}
